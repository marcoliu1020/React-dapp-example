...
const ERC721Assignment = () => {
  ...
  useEffect(() => {
    ...
    const getMintPrice = async () => {
      ...
      const _ether = ethers.utils.formatEther(_mintPrice)
      setPrice(_ether)
    }
  ...
  useEffect(() => {
    ...
    const getAccountBalance = async () => {
      const _balance = await contract.balanceOf();
  ...
  const onMint = async () => {
     ...
     */
    if (!mintPrice) {
      console.log("no mint price")
      return;
    }

    if (contract && currentAccount) {
      const _wei = ethers.utils.parseEther(mintPrice)
      await contract.mint({ value: _wei, from: currentAccount});
    }
  };
  ...
  useEffect(() => {
    ...
    if (contract) {
      let interval = window.setInterval(() => {
          ...
          .then(totalSupply => setTotalSupply(totalSupply.toString()))
      }, 1000)
  ...
  useEffect(() => {
    if (totalSupply > 0) {
      const items = [...Array(parseInt(totalSupply))].map((e,i) =>
        <OwnerListItem key={i} tokenId={i} contract={contract} />
      )
      // console.log(items)
      setListItems(items)
    }
